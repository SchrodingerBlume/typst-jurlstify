...
<p align="center">
  ...
  <a href="manual/manual.pdf"><img src="https://img.shields.io/badge/📖_manual-PDF-blueviolet" alt="manual (PDF)"></a>
</p>
...
📖 **Full API reference:** the [manual (PDF)](manual/manual.pdf), rendered from [`manual/manual.typ`](manual/manual.typ).
...
See the [manual](manual/manual.pdf) for every parameter's type, default, and description. The three default character sets are exported as `break-chars-default` / `big-break-chars-default` / `no-break-chars-default` for extending.
...
📖 **完整 API 参考:** 见[手册 PDF](manual/manual.pdf),由 [`manual/manual.typ`](manual/manual.typ) 渲染。
...
每个参数的类型、默认值、说明见[手册](manual/manual.pdf)。三个默认字符集导出为 `break-chars-default` / `big-break-chars-default` / `no-break-chars-default`,可用于扩展。
